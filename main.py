...
from orcamento_items import configurar_orcamento_ui, navegar_item_orcamento, atualizar_custos_e_precos_itens, calcular_preco_final_orcamento  # Função para navegar com setas up/down entre os itens do orçamento dentro do separador 'Orcamento de Items'
...
from menu_grupos_def_pecas import atualizar_grupos_pecas # Atualiza os 6 QListWidget com os nomes das peças lidos do ficheiro Excel 'TAB_DEF_PECAS.XLSX'.
from tabela_def_pecas_items import configurar_selecao_qt_lists # Configura os QListWidget para permitir a seleção de múltiplos itens e alternar o estado de check ao clicar na linha.
from tabela_def_pecas_items import conectar_inserir_def_pecas_tab_items, inserir_pecas_selecionadas, setup_context_menu, on_item_changed_def_pecas # Adiciona linhas na tabela "tab_def_pecas" a partir de peças selecionadas nos 6 GRUPOS QListWidget
from modulo_orquestrador import  atualizar_tudo # Importe a função calcular_orlas do módulo calculo_orlas e atualizar os preços e materias primas tab_def_pecas.

from controle_edicao_manual_blk import conectar_eventos_edicao_manual, on_cell_changed_for_blk_logic # Importar a função de conexão da lógica BLK
from utils import safe_item_text, set_item # Função para obter o texto de um item de tabela, retornando uma string vazia se o item for None
...
import modulo_gestao_modulos_db # Modulo que trada Este módulo é responsável por todas as interações com a base de dados referentes à gravação, carregamento, e gestão de "Módulos Guardados".
...
class MainApp(QMainWindow):
    def __init__(self):
        ...
        self.ui.tab_artigos_11.setSelectionMode(self.ui.tab_artigos_11.SingleSelection)
        self.ui.tab_artigos_11.setStyleSheet("QTreeWidget::item:selected { background-color: #F7DC6F; }")
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro de Inicialização", f"Falha ao configurar o módulo de Clientes:\n{e}")
        ...
        try:
            ...
            self.ui.botao_up_item.clicked.connect(lambda: navegar_item_orcamento(self, -1))
            self.ui.botao_down_item.clicked.connect(lambda: navegar_item_orcamento(self, 1))
            #print("[INFO] Módulo Orçamento Items conectado.")
        except Exception as e:
            print(f"[ERRO] Falha ao conectar módulo Orçamento Items: {e}")
            QMessageBox.critical(self, "Erro de Inicialização", f"Falha ao configurar o separador de Itens de Orçamento:\n{e}")
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro de Inicialização", f"Falha ao configurar o módulo de Configurações:\n{e}")
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro de Inicialização", f"Falha ao configurar o módulo de Matérias-Primas:\n{e}")
        ...
        try:
            configurar_botoes_dados_gerais(self) # Conecta botões comuns de guardar/importar/limpar
        ...
        try:
            ...
            print(f"[ERRO] Falha ao configurar Dados Gerais - Sistemas Correr: {e}")

        try:
            configurar_acabamentos_ui(self.ui)
            #print("[INFO] Módulo Dados Gerais - Acabamentos configurado.")
        except Exception as e:
            print(f"[ERRO] Falha ao configurar Dados Gerais - Acabamentos: {e}") 
        ...
        try:
            ...
            inicializar_dados_items_material(self)  # Aqui você conecta o botão de guardar dados dos itens
        ...
        try:
            ...
            inicializar_dados_items_ferragens(self)  # Aqui você conecta o botão de guardar dados dos itens
        ...
        try:
            ...
            inicializar_dados_items_sistemas_correr(self)  # Aqui você conecta o botão de guardar dados dos itens
        ...
        try:
            ...
            inicializar_dados_items_acabamentos(self)  # Aqui você conecta o botão de guardar dados dos itens
        ...
        try:
            ...
            atualizar_grupos_pecas(self.ui) # Preenche os QListWidget com opções do Excel
            conectar_inserir_def_pecas_tab_items(self.ui) # Conecta botão "Inserir Peças"
            setup_context_menu(self.ui, None) # Configura menu de contexto e delegates
            configurar_selecao_qt_lists(self.ui) # Configura clique simples nos QListWidget
            ...
            table_def_pecas.cellChanged.connect(lambda r, c: on_cell_changed_for_blk_logic(self.ui, r, c))
            print("[INFO Main] Sinal cellChanged conectado a on_cell_changed_for_blk_logic.")
        ...
        try:
            self.ui.Inserir_Pecas_Selecionadas.clicked.connect(lambda: inserir_pecas_selecionadas(self.ui))
        ...
        self.ui.pushButton_atualiza_preco_final.clicked.connect(lambda: calcular_preco_final_orcamento(self.ui))
        ...
        try:
            ...
            self.ui.Guardar_Dados_Items.clicked.connect(
                lambda: (
                    ...
                    modulo_dados_definicoes.salvar_dados_modulo_medidas(self.ui),
        ...
        try:
            ...
            if hasattr(self.ui, 'Guardar_Modulo'):
                self.ui.Guardar_Modulo.clicked.connect(self.on_guardar_modulo_clicked)
            ...
            if hasattr(self.ui, 'Importar_Modulo'):
                self.ui.Importar_Modulo.clicked.connect(self.on_importar_modulo_clicked)
            ...
            if hasattr(self.ui, 'Gerir_Modulo'): 
                self.ui.Gerir_Modulo.clicked.connect(self.on_gerir_modulos_clicked)
                print("[INFO Main] Botão 'Gerir_Modulo' conectado.")
            else:
                # Se o nome do botão for diferente, ajuste a string aqui também
                print("[AVISO Main] Botão 'Gerir_Modulo' (ou nome esperado) não encontrado na UI. Funcionalidade de gestão não conectada.")

            # print("[INFO Main] Botões de gestão de módulos parcialmente conectados.") # Pode remover ou ajustar esta linha

        except AttributeError as ae:
            print(f"[ERRO Main ATTRIBUTE] Falha ao conectar botões de gestão de módulos: {ae}. Verifique os nomes dos botões na UI.")
        except Exception as e:
            print(f"[ERRO Main] Falha geral ao conectar botões de gestão de módulos: {e}")
        ...
        try:
            ...
            self.ui.Tab_Ferragens.itemChanged.connect(on_item_changed_ferragens)
            self.ui.Tab_Sistemas_Correr.itemChanged.connect(on_item_changed_sistemas_correr)
            self.ui.Tab_Acabamentos.itemChanged.connect(on_item_changed_acabamentos)
            #print("[INFO] Sinais itemChanged das tabelas de Dados Gerais conectados.")
        except Exception as e:
            print(f"[ERRO] Falha ao conectar sinais itemChanged de Dados Gerais: {e}")
        ...
        try:
            self.ui.importar_dados_gerais_4_separadores.clicked.connect(self.abrirImportarDialog)
    ...
    def abrirImportarDialog(self):
        ...
        dialog = ImportarDadosGeralDialog(self, self) # Passa a instância de MainApp como parent
        dialog.exec_()
    ...
    def configurar_base_dados(self):
        ...
        self.ui.lineEdit_base_dados.textChanged.connect(lambda: set_db_path(self.ui.lineEdit_base_dados))
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro Crítico de Base de Dados", f"Falha na inicialização da tabela de Clientes:\n{e}")
    ...
    def on_guardar_modulo_clicked(self):
        ...
        for row in range(table.rowCount()):
            ...
            if item_chk and item_chk.checkState() == Qt.Checked:
                linhas_selecionadas_indices.append(row) # Guardar o índice original da linha
                
                peca_dados = {
                    "ordem_peca": len(pecas_para_gravar), # Ordem baseada na sequência de seleção
                    "descricao_livre_peca": safe_item_text(table, row, IDX_DESCRICAO_LIVRE),
                    "def_peca_peca": safe_item_text(table, row, IDX_DEF_PECA_MOD),
                    "qt_und_peca": safe_item_text(table, row, IDX_QT_UND_MOD), # Guardar como texto
                    "comp_peca": safe_item_text(table, row, IDX_COMP_MOD),       # Guardar como texto
                    "larg_peca": safe_item_text(table, row, IDX_LARG_MOD),       # Guardar como texto
                    "esp_peca": safe_item_text(table, row, IDX_ESP_MOD),        # Guardar como texto
                ...
                if item_def_peca and item_def_peca.data(Qt.UserRole):
                    ...
                    peca_dados["grupo_peca"] = "" # Default se não houver grupo
        ...
        print(f"[INFO Main] {len(pecas_para_gravar)} peças marcadas para gravação no módulo.")
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                print("[INFO Main] Módulo gravado com sucesso (ou operação concluída no diálogo).")
                ...
                print("[INFO Main] Operação de gravar módulo cancelada pelo utilizador.")
        except ImportError:
            QMessageBox.critical(self, "Erro", "O ficheiro 'dialogo_gravar_modulo.py' não foi encontrado.")
            print("[ERRO Main] Falha ao importar DialogoGravarModulo.")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao abrir o diálogo de gravação de módulo: {e}")
            print(f"[ERRO Main] Erro ao instanciar/executar DialogoGravarModulo: {e}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                if id_modulo_a_importar is None:
                    QMessageBox.warning(self, "Seleção Inválida", "Nenhum módulo foi selecionado para importação.")
                    return

                print(f"[INFO Main] Importando peças do módulo ID: {id_modulo_a_importar}")
                pecas_do_modulo = modulo_gestao_modulos_db.obter_pecas_de_modulo(id_modulo_a_importar)

                if not pecas_do_modulo:
                    QMessageBox.information(self, "Módulo Vazio", f"O módulo selecionado (ID: {id_modulo_a_importar}) não contém peças.")
                    return
                ...
                try:
                    for peca_data in pecas_do_modulo:
                        ...
                        set_item(table, new_row, 1, peca_data.get("descricao_livre_peca", ""))
                        
                        item_def_peca = QTableWidgetItem(peca_data.get("def_peca_peca", ""))
                        item_def_peca.setData(Qt.UserRole, peca_data.get("grupo_peca", ""))
                        table.setItem(new_row, IDX_DEF_PECA, item_def_peca) # Usar constante
                        
                        set_item(table, new_row, 5, peca_data.get("qt_und_peca", "1"))
                        set_item(table, new_row, 6, peca_data.get("comp_peca", ""))
                        set_item(table, new_row, 7, peca_data.get("larg_peca", ""))
                        set_item(table, new_row, 8, peca_data.get("esp_peca", ""))

                        for col_chk_idx in [9, 10, 11, 12, 53, 59, 60]:
                            chk = QTableWidgetItem()
                            chk.setFlags(Qt.ItemIsUserCheckable | Qt.ItemIsEnabled | Qt.ItemIsSelectable)
                            chk.setCheckState(Qt.Unchecked) # Por padrão, módulos importados não vêm com BLK ativo
                            table.setItem(new_row, col_chk_idx, chk)
                        
                        set_item(table, new_row, 13, peca_data.get("mat_default_peca", ""))
                        set_item(table, new_row, 14, peca_data.get("tab_default_peca", ""))
                        
                        set_item(table, new_row, 15, self.ui.lineEdit_item_orcamento.text().strip())
                        set_item(table, new_row, 16, self.ui.lineEdit_num_orcamento.text().strip())
                        set_item(table, new_row, 17, self.ui.lineEdit_versao_orcamento.text().strip())
                        
                        set_item(table, new_row, 24, peca_data.get("und_peca", ""))

                        btn = QPushButton("Escolher")
                        # Correção na conexão do botão para usar self.on_mp_button_clicked
                        btn.clicked.connect(lambda _, r=new_row: self.on_mp_button_clicked(r, "tab_def_pecas"))
                        ...
                        set_item(table, new_row, IDX_COMP_ASS_1, peca_data.get("comp_ass_1_peca", ""))
                        set_item(table, new_row, IDX_COMP_ASS_2, peca_data.get("comp_ass_2_peca", ""))
                        set_item(table, new_row, IDX_COMP_ASS_3, peca_data.get("comp_ass_3_peca", ""))


                        print(f"  Peça '{peca_data.get('def_peca_peca')}' adicionada à tab_def_pecas na linha {new_row+1}")
                    ...
                    for r_imp in range(primeira_linha_importada, table.rowCount()):
                        ...
                        if not item_def_peca_imp: continue
                        ...
                        if texto_def_peca_imp == "MODULO":
                            ...
                            if e_associado_neste_bloco:
                                ...
                                comps_da_ultima_principal_importada.clear()
                                c1 = safe_item_text(table, r_imp, IDX_COMP_ASS_1).strip().upper()
                                c2 = safe_item_text(table, r_imp, IDX_COMP_ASS_2).strip().upper()
                                c3 = safe_item_text(table, r_imp, IDX_COMP_ASS_3).strip().upper()
                                if c1: comps_da_ultima_principal_importada.add(c1)
                                if c2: comps_da_ultima_principal_importada.add(c2)
                                if c3: comps_da_ultima_principal_importada.add(c3)
                        ...
                        for c_col in range(table.columnCount()):
                            item_celula = table.item(r_imp, c_col) or QTableWidgetItem()
                            if table.item(r_imp, c_col) is None: table.setItem(r_imp, c_col, item_celula)
                            item_celula.setBackground(cor_a_aplicar_linha_imp)
                    print("[INFO Main] Cores iniciais aplicadas às linhas do módulo importado.")
                ...
                update_ids(table)
                print("[INFO Main] Módulo importado. Chamando o orquestrador para processar...")
                QTimer.singleShot(10, lambda: atualizar_tudo(self.ui)) # Pequeno delay para garantir que a UI atualiza antes do processamento pesado
            ...
            QMessageBox.critical(self, "Erro de Importação de Módulo", f"Erro ao carregar componentes do diálogo: {ie}\nVerifique se 'dialogo_importar_modulo.py' está no local correto.")
            print(f"[ERRO Main] Falha ao importar DialogoImportarModulo ou suas dependências: {ie}")
        except Exception as e:
            QMessageBox.critical(self, "Erro ao Importar", f"Ocorreu um erro ao importar o módulo: {e}")
    ...
    def on_gerir_modulos_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro", "O ficheiro 'dialogo_gerir_modulos.py' não foi encontrado.")
            print("[ERRO Main] Falha ao importar DialogoGerirModulos.")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao abrir o diálogo de gestão de módulos: {e}")
            print(f"[ERRO Main] Erro ao instanciar/executar DialogoGerirModulos: {e}")
            import traceback
            traceback.print_exc() 
