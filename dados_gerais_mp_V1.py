...
#from utils import adicionar_menu_limpar # Importa a função para adicionar o menu de limpar linhas
from utils import adicionar_menu_dados_gerais, copiar_linha_tabela, safe_item_text
...
def configurar_botoes_dados_gerais(main_window):
    ...
    try:
        ...
        adicionar_menu_dados_gerais(
            main_window.ui.Tab_Material,
            lambda: limpar_linha_por_tab(main_window, "materiais"),
            lambda: aplicar_linha_para_items(main_window, "materiais")
        )
    ...
    try:
        ...
        adicionar_menu_dados_gerais(
            ...
            lambda: aplicar_linha_para_items(main_window, "ferragens")
        )
    except Exception as e:
    ...
    try:
        ...
        adicionar_menu_dados_gerais(
            ...
            lambda: aplicar_linha_para_items(main_window, "sistemas_correr")
        )
    except Exception as e:
    ...
    try:
        ...
        adicionar_menu_dados_gerais(
            ...
            lambda: aplicar_linha_para_items(main_window, "acabamentos")
        )
    except Exception as e:
...
def aplicar_linha_para_items(main_window, nome_tabela):
    """Copia a linha selecionada dos Dados Gerais para a tabela dos itens e atualiza Tab_Def_Pecas."""
    ui = main_window.ui

    if nome_tabela == "materiais":
        tbl_geral, tbl_item, tab_nome = ui.Tab_Material, ui.Tab_Material_11, "Tab_Material_11"
    elif nome_tabela == "ferragens":
        tbl_geral, tbl_item, tab_nome = ui.Tab_Ferragens, ui.Tab_Ferragens_11, "Tab_Ferragens_11"
    elif nome_tabela == "sistemas_correr":
        tbl_geral, tbl_item, tab_nome = ui.Tab_Sistemas_Correr, ui.Tab_Sistemas_Correr_11, "Tab_Sistemas_Correr_11"
    elif nome_tabela == "acabamentos":
        tbl_geral, tbl_item, tab_nome = ui.Tab_Acabamentos, ui.Tab_Acabamentos_12, "Tab_Acabamentos_12"
    else:
        return

    sel = tbl_geral.selectionModel().selectedRows()
    if not sel:
        QMessageBox.warning(main_window, "Aviso", "Nenhuma linha selecionada!")
        return
    ...
    for idx in sel:
        row = idx.row()
        copiar_linha_tabela(tbl_geral, tbl_item, row)

        nome_mat = tbl_geral.verticalHeaderItem(row).text() if tbl_geral.verticalHeaderItem(row) else ""
        tab = ui.tab_def_pecas
        for r in range(tab.rowCount()):
            if safe_item_text(tab, r, 13) == nome_mat and safe_item_text(tab, r, 14) == tab_nome:
                linhas_afetadas.append(r + 1)

    if linhas_afetadas:
        linhas_txt = ", ".join(map(str, sorted(set(linhas_afetadas))))
        msg = f"Atualizar linhas {linhas_txt} da Tab_Def_Pecas?"
        if QMessageBox.question(main_window, "Confirmar", msg, QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes) == QMessageBox.No:
            return
