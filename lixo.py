...
def gera_pdf(ui: QtWidgets.QWidget, caminho: str) -> None:
    ...
    elems.append(table)
    elems.append(Spacer(1, 12))
    subtotal = ui.label_subtotal_2.text().replace("Subtotal:", "Subtotal:").strip()
    iva = ui.label_iva_2.text().replace("IVA:", "IVA:").strip()
    total_geral = ui.label_total_geral_2.text().replace("Total Geral:", "Total Geral:").strip()
    total_qt = ui.label_total_qt_2.text().replace("Total QT:", "Total QT:").strip()

    # Obter apenas os valores numéricos e adicionar o símbolo do euro, caso necessário
    def add_euro(valor):
        valor = valor.strip()
        return valor if "€" in valor else valor + "€"

    subtotal_val = add_euro(subtotal.split(":")[-1].strip())
    iva_val = iva.split(":")[-1].strip()
    total_geral_val = add_euro(total_geral.split(":")[-1].strip())
    total_qt_val = total_qt.split(":")[-1].strip()

    # Criar um pequeno quadro para os totais
    totais_data = [
        ["", ""],
        ["Total QT:", total_qt_val],
        ["Subtotal:", subtotal_val],
        ["IVA (23%):", iva_val],
        ["<b>Total Geral:</b>", f"<b>{total_geral_val}</b>"]
    ]
