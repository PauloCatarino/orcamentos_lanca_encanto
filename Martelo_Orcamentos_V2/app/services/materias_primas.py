...
DEFAULT_MATERIAS_BASE_PATH = r"\\server_le\_Lanca_Encanto\LancaEncanto\Dep._Orcamentos\Base_Dados_Orcamento"
...
def import_materias_primas(db: Session, base_path: Optional[str] = None) -> int:
    ...
    # Algumas versões do Excel usam nomes e/ou ordem diferentes.
    # Fazemos match por cabeçalho (com aliases) em vez de validar lista completa.
    header_aliases: Mapping[str, Sequence[str]] = {
        "margem": ("MARGEM", "MRG", "MRG_(+)"),
        "desconto": ("DESCONTO", "DESC2", "DESC2(-)", "DESC2_(-)"),
        "orl_0_4": ("ORL_0.4", "ORL_0_4", "CORRESP_ORLA_0_4", "CORESP_ORLA_0_4"),
        "orl_1_0": ("ORL_1.0", "ORL_1_0", "CORRESP_ORLA_1_0", "CORESP_ORLA_1_0"),
    }
    ...
    try:
        ...
        for meta in COLUMN_DEFS:
            candidates = [meta.header, *header_aliases.get(meta.attr, ())]
            found: int | None = None
            for cand in candidates:
                idx = header_index.get(_norm_header(cand))
                if idx is not None:
                    found = idx
                    break
            col_indices.append(found)

        if col_indices[0] is None:
            raise ValueError(
                "Coluna obrigatória 'ID_MP' não encontrada no cabeçalho do Excel.\n"
                f"Cabeçalho encontrado: {header_values}"
            )

        db.query(MateriaPrima).delete()

        inserted = 0
        for row_values in ws.iter_rows(min_row=6, max_col=ws.max_column, values_only=True):
            values: list[object | None] = []
            for idx in col_indices:
                if idx is None or idx >= len(row_values):
                    values.append(None)
                else:
                    values.append(row_values[idx])
            ...
            if stock_val is None:
                ...
                data["stock"] = 1 if stock_decimal >= 1 else 0
