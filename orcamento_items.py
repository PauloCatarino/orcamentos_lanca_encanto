...
def _validate_percentage_input(line_edit, min_val, max_val):
    ...
    try:
        ...
        formatted_text = f"{value:.2f}%" # Formatar como inteiro para percentagem
...
def calcular_preco_final_orcamento(ui):
    ...
    target_price = converter_texto_para_valor(target_price_str, "moeda") if target_price_str else 0.0
    ...
    sum_custo_produzido_vezes_qt_total = 0.0 # Σ (Custo Produzido * QT) para o denominador
    sum_custo_base_e_outros_ajustes_vezes_qt_total = 0.0 # Σ (Custo Produzido * (1 + Admin_Perc + Aj1_Perc + Aj2_Perc) * QT) para o numerador
    ...
    try:
        for row_idx in range(tbl.rowCount()):
            try:
                ...
                preco_total_items.append(converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_PRECO_TOTAL), "moeda"))
                
                # Coleta de dados para a nova fórmula da margem global
                custo_produzido_item = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_CUSTO_PRODUZIDO), "moeda")
                qt_item = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_QT), "moeda")
                if qt_item <= 0: qt_item = 1.0
                ...
                custos_admin_perc_effective = custos_admin_perc_cell if abs(custos_admin_perc_cell - global_custos_admin_perc) > 0.001 else global_custos_admin_perc
                ajustes1_perc_effective = ajustes1_perc_cell if abs(ajustes1_perc_cell - global_ajustes1_perc) > 0.001 else global_ajustes1_perc
                ajustes2_perc_effective = ajustes2_perc_cell if abs(ajustes2_perc_cell - global_ajustes2_perc) > 0.001 else global_ajustes2_perc
                ...
                custo_base_e_outros_ajustes_unit = custo_produzido_item * (1 + custos_admin_perc_effective + ajustes1_perc_effective + ajustes2_perc_effective)
                sum_custo_base_e_outros_ajustes_vezes_qt_total += custo_base_e_outros_ajustes_unit * qt_item

            except Exception as e:
                print(f"[ERRO] Falha na leitura/conversão de dados de item na linha {row_idx}: {e}")
    ...
    if target_price > 0: # Se um objetivo de preço final foi definido
        print(f"[INFO] Objetivo de preço final: {formatar_valor_moeda(target_price)}")
        ...
        new_margin_perc = (target_price - sum_custo_base_e_outros_ajustes_vezes_qt_total) / sum_custo_produzido_vezes_qt_total
        ...
        global _editando_programaticamente
        _editando_programaticamente = True # Bloqueia sinais durante atualizações programáticas
        try:
            # Atualizar o lineEdit global de margem de lucro
            ui.lineEdit_margem_lucro.setText(formatar_valor_percentual(new_margin_perc))
    ...
    for row_idx in range(tbl.rowCount()):
        current_preco_total_cell_text = _get_cell_text(tbl, row_idx, COL_PRECO_TOTAL)
        val = converter_texto_para_valor(current_preco_total_cell_text, "moeda")
    ...
    ui.lineEdit_preco_final_orcamento.setText(formatar_valor_moeda(final_sum_preco_total))
