...
from utils import (formatar_valor_moeda, formatar_valor_percentual, converter_texto_para_valor, validar_expressao_modulo, set_item, safe_item_text, VARIAVEIS_VALIDAS)
...
from aplicar_formatacao_visual_blk import aplicar_ou_limpar_formatacao_blk # Importa a função para aplicar formatação visual ao BLK coluna 12 'Bloqueado'

from modulo_componentes_associados import COLOR_ASSOCIATED_BG, COLOR_MODULO_BG # Importa as cores para os componentes associados e MODULO
from modulo_orquestrador import atualizar_tudo # Importa a função para o botão "Escolher" (MP) na tabela
...
def inserir_pecas_selecionadas(ui):
    ...
    actualizar_ids_num_orc_ver_orc_tab_modulo_medidas( ui, valor_ids, valor_num_orc, valor_ver_orc )
    ...
    setup_context_menu(ui, None) # Configura/reaplica menu de contexto e delegates (incluindo DefPecaDelegate)
    aplicar_combobox_mat_default(ui) # Aplica o ComboBox Mat_Default (AGORA, depois de inserir Mat_Default/Tab_Default)
...
def inserir_linha_componente(ui, texto_peca):
    ...
    """
    print(f"[INFO] Preparando para inserir linha básica para componente associado: '{texto_peca}'...")
    ...
    if not grupo_encontrado:
        print(f"[Aviso] Componente associado '{texto_peca}' não encontrado em nenhum grupo QListWidget. Não será inserido.")
    ...
    item_def.setFlags(item_def.flags() | Qt.ItemIsEditable) # Mantém editável para delegate
    item_def.setData(Qt.UserRole, grupo_encontrado) # Armazena o grupo
    item_def.setBackground(QColor(230, 240, 255))  # 🟦 cor azul clara para identificar componentes associados
    ...
    for col in [9, 10, 11, 12, 53, 59, 60]: # MPs, MO, Orla, BLK, GRAVAR_MODULO, ACB_SUP, ACB_INF
        chk = QTableWidgetItem() # Cria novo item checkbox
        chk.setFlags(Qt.ItemIsUserCheckable | Qt.ItemIsEnabled | Qt.ItemIsSelectable) # Adicionado IsSelectable
        chk.setCheckState(Qt.Unchecked)
    ...
    set_item(table, new_row, 15, ui.lineEdit_item_orcamento.text().strip()) # Usa set_item
    set_item(table, new_row, 16, ui.lineEdit_num_orcamento.text().strip()) # Usa set_item
    set_item(table, new_row, 17, ui.lineEdit_versao_orcamento.text().strip()) # Usa set_item

    # Colunas 18-32 (ref_le a esp_mp) e 34-81 SÃO DEIXADAS VAZIAS
    for col in range(18, 82): # 18 até 81 (inclusive)
         if col == 33: continue # Salta a coluna 33 onde vai o botão MP
         set_item(table, new_row, col, "") # Usa set_item

    # Botão "Escolher" (col 33)
    btn = QPushButton("Escolher")
    # Conecta o botão à função de seleção de material, passando a linha atual
    btn.clicked.connect(lambda _, r=new_row: on_mp_button_clicked(ui, r, "tab_def_pecas"))
...
def atualizar_dados_def_peca(ui, row):
    ...
    if blk_item and blk_item.checkState() == Qt.Checked:
        print(f"[INFO] Linha {row+1} está bloqueada (BLK=True). Saltando consulta à BD para dados adicionais.")
    ...
    if tab_default not in db_mapping or tab_default not in id_mapping:
         print(f"[AVISO] Linha {row+1}: Mapeamento não encontrado para Tab_Default '{tab_default}'. Saltando atualização de dados adicionais.")
    ...
    try:
        ...
        with obter_cursor() as cursor:
            ...
            query = (
                ...
                f"WHERE `num_orc`=%s AND `ver_orc`=%s AND `{id_column}`=%s AND `material`=%s" # Usar backticks para nomes de colunas
            )
            ...
            cursor.execute(query, (valor_num_orc, valor_ver_orc, valor_ids, mat_default))
            ...
            try:
                for idx_res, col_target_ui in mapping.items():
                    ...
                    if valor_bd is not None:
                        ...
                        elif col_target_ui in {22, 23, 25}: # des1plus, des2_minus, desp (Percentual)
                ...
                esp_valor_bd = resultado[IDX_ESP_MP_RESULT] # Obter esp_mp do resultado
        ...
        print(f"[ERRO DB] Linha {row+1}: Erro ao buscar dados da peça em {db_table}: {db_err}")
        # Não mostrar QMessageBox aqui, para não travar o orquestrador se houver múltiplos erros
    except Exception as e:
        # Outros erros inesperados
        print(f"[ERRO INESPERADO] Linha {row+1}: Erro ao atualizar dados da peça: {e}")
        import traceback
        traceback.print_exc()
...
def update_ids(table):
    ...
    table.blockSignals(True)  # Evita emissões de itemChanged durante a renumeração
    try:
        for row in range(table.rowCount()):
            # Obtém o item existente ou cria um se não existir (usando set_item)
            set_item(table, row, 0, str(row + 1)) # Usa set_item para definir o texto e garantir o item
...
class DefPecaDelegate(QStyledItemDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        editor.textActivated.connect(lambda text: self.handle_selection_committed(editor, index, text))
    ...
    def setModelData(self, editor, model, index):
        ...
        if blk_item and blk_item.checkState() == Qt.Checked:
             ...
             print(f"[INFO] Delegate DefPeca: setModelData - Linha {row+1} está bloqueada. Não salvando mudança.")
    ...
    def handle_selection_committed(self, editor, index, text):
        ...
        if widget:
            for i in range(widget.count()):
             ...
             print(f"[AVISO] Delegate DefPeca: Grupo '{grupo}' não encontrado nos mapeamentos QListWidget.")
        ...
        aplicar_combobox_mat_default(self.ui) # Pode ser ineficiente, mas garante o ComboBox correto.
        ...
        from modulo_orquestrador import atualizar_tudo
        print(f"[INFO] Delegate DefPeca: Chamando orquestrador após alteração na linha {row+1}.")
        ...
        if event.type() == QEvent.KeyPress:
            ...
            if key in (Qt.Key_Return, Qt.Key_Enter, Qt.Key_Left, Qt.Key_Right):
                ...
                if 0 <= col < table.columnCount():
                    ...
                    QTimer.singleShot(0, lambda: table.setCurrentCell(row, col))
                ...
                if col in [IDX_QT_MOD, IDX_COMP, IDX_LARG, IDX_ESP]:
                    QTimer.singleShot(10, lambda: atualizar_tudo(self.ui)) # Pequeno delay
...
def inserir_componentes_associados_para_linha(ui, linha_principal):
    ...
    if linha_principal < 0 or linha_principal >= table.rowCount():
        print(f"[Erro] Linha principal inválida para componentes associados: {linha_principal}")
    ...
    if blk_item and blk_item.checkState() == Qt.Checked:
        ...
        print(f"[INFO] Linha {linha_principal+1} (Principal) está bloqueada (BLK=True). Saltando busca de componentes associados no Excel.")
    ...
    if not os.path.exists(excel_file):
        print("[Erro] Ficheiro TAB_DEF_PECAS.XLSX não encontrado para componentes associados.")
    ...
    try:
        ...
        print(f"[ERRO] Erro ao ler o ficheiro Excel '{excel_file}' para associados: {e}")
    ...
    if "DEF_PECA" in df.columns:
         df['DEF_PECA_CLEAN'] = df['DEF_PECA'].astype(str).str.strip().str.upper()
    ...
    row_excel_data = linha_excel.iloc[0] # Pega a primeira linha correspondente
    ...
    table.blockSignals(True) # Bloqueia sinais durante o preenchimento na linha UI
    try:
        for i, col_name in enumerate(comp_col_names):
            # Obtém o valor do DataFrame, tratando NaN e convertendo para string limpa
            val = row_excel_data.get(col_name) # Use .get() para evitar KeyError se coluna faltar
            val_str = str(val).strip() if pd.notna(val) and isinstance(val, str) else "" # Trata NaN e tipos não string
    ...
    if item_def:
         ...
         if current_color.name() != COLOR_MODULO_BG.name(): # Se não for a cor cinza do MODULO
             ...
             if comps_a_inserir:
                item_def.setBackground(QColor(180, 200, 255)) # Azul mais escuro para principal com associados
             else:
              ...
              if comps_a_inserir:
                  ...
                  item_def.setToolTip(f"MODULO com componentes associados: {', '.join(comps_a_inserir)}.\n" + old_tooltip)
...
def configurar_selecao_qt_lists(ui):
        ...
        def ao_clicar_item(item):
            ...
            if item.flags() & Qt.ItemIsUserCheckable:
                ...
                item.setCheckState(estado)
                 ...
                 print(f"[INFO] Item '{item.text()}' não é chekable.") # Debug/Info se um item não é checkable
    ...
    for lw in list_widgets_a_configurar:
        if lw is not None: # Verifica se o objeto UI foi criado corretamente
             ...
             print("[AVISO] Objeto QListWidget não encontrado na UI durante a configuração.")
    ...
    try:
        ...
        if col in colunas_para_formatar:
            ...
            try:
                ...
                valor_num = converter_texto_para_valor(texto_atual, tipo_formato)
...
def on_mp_button_clicked(ui, row, nome_tabela):
    ...
    if escolher_material_item(ui, row):
        ...
        aplicar_ou_limpar_formatacao_blk(ui.tab_def_pecas, row, aplicar=True, origem_pliq_tooltip="escolher")
        ...
        QTimer.singleShot(10, lambda: atualizar_tudo(ui))# Usar QTimer para garantir que a atualização ocorre após o evento atual
...
def escolher_material_item(ui, linha_tab):
    ...
    if result == dialog.Accepted:
        ...
        try:
            ...
            col_map = {
                'ref_le': (3, 18),                  # REF_PHC (MP) -> ref_le (Peca)
                'descricao_no_orcamento': (5, 19),  # Descrição Resumida (MP) -> descricao_no_orcamento (Peca)
                'ptab': (6, 20),                    # Preço Tabela (MP) -> ptab (Peca)
                'pliq': (6, 21),                    # Preço Tabela (MP) -> pliq (Peca) - PliQ será recalculado depois
                'des1plus': (7, 22),                # Desc. Adicional (MP) -> des1plus (Peca)
                'des2_minus': (8, 23),              # Desc. Percentual (MP) -> des2_minus (Peca)
                'und': (10, 24),                    # Unidade (MP) -> und (Peca)
                'desp': (11, 25),                   # Desperdício (%) (MP) -> desp (Peca)
                'corres_orla_0_4': (16, 26),        # Ref. Orla Fina (MP) -> corres_orla_0_4 (Peca)
                'corres_orla_1_0': (17, 27),        # Ref. Orla Grossa (MP) -> corres_orla_1_0 (Peca)
                'tipo': (13, 28),                   # Tipo (MP) -> tipo (Peca)             'tipo': (18, 28),
                'familia': (14, 29),                # Família (MP) -> familia (Peca)      'familia': (19, 29),
                'comp_mp': (19, 30),                # Comp. MP (MP) -> comp_mp (Peca)      'comp_mp': (20, 30),
                'larg_mp': (20, 31),                # Larg. MP (MP) -> larg_mp (Peca)      'larg_mp': (21, 31),  
                'esp_mp': (12, 32)                  # Esp. MP (MP) -> esp_mp (Peca)
            ...
            col_map['esp_peca_ui'] = (12, 8) # Esp. MP (MP) -> Esp (Peca UI col 8)
            ...
            for campo, (src_idx, tgt_idx) in col_map.items():
                ...
                texto_formatado = ""
                if tgt_idx in {20, 21}: texto_formatado = formatar_valor_moeda(valor_num)
                elif tgt_idx in {22, 23, 25}: texto_formatado = formatar_valor_percentual(valor_num)
                else: texto_formatado = valor_texto
                set_item(tbl, linha_tab, tgt_idx, texto_formatado)# Usa set_item com valor JÁ FORMATADO
            ...
            des1plus_text_copiado = safe_item_text(tbl, linha_tab, IDX_DES1PLUS)
            des2minus_text_copiado = safe_item_text(tbl, linha_tab, IDX_DES1MINUS)
            ptab_valor_num = converter_texto_para_valor(ptab_text_copiado, "moeda")
            des1plus_valor_num = converter_texto_para_valor(des1plus_text_copiado, "percentual")
            des2minus_valor_num = converter_texto_para_valor(des2minus_text_copiado, "percentual")
            novo_pliq_num = round(ptab_valor_num * (1 + des1plus_valor_num) * (1 - des2minus_valor_num), 2)
            set_item(tbl, linha_tab, IDX_PLIQ, formatar_valor_moeda(novo_pliq_num))
            ...
            QMessageBox.critical(tbl.window(), "Erro", f"Erro ao aplicar material: {e}")
...
def conectar_inserir_def_pecas_tab_items(ui):
    ...
    try:
        ...
        pass
    ui.Inserir_Pecas_Selecionadas.clicked.connect(lambda: inserir_pecas_selecionadas(ui))
...
def actualizar_ids_num_orc_ver_orc_tab_modulo_medidas(ui, ids_val, num_orc_val, ver_orc_val):
    ...
    if ver_orc_val is not None:
        try:
            ...
            ver_orc_formatado = str(ver_orc_val).strip() if str(ver_orc_val).strip() else "00"
    # === FIM MODIFICAÇÃO ===
    ...
    try:
        ...
        if item_ids: item_ids.setFlags(item_ids.flags() & ~Qt.ItemIsEditable)

        # Coluna 16: num_orc
        set_item(tbl, row, 16, num_orc_val)
        item_num = tbl.item(row, 16)
        if item_num: item_num.setFlags(item_num.flags() & ~Qt.ItemIsEditable)

        # Coluna 17: ver_orc
        set_item(tbl, row, 17, ver_orc_formatado) # Usa a versão formatada
        item_ver = tbl.item(row, 17)
        if item_ver: item_ver.setFlags(item_ver.flags() & ~Qt.ItemIsEditable)
        ...
        print(f"[INFO] Tab_modulo_medidas chaves atualizadas: ids='{ids_val}', num='{num_orc_val}', ver='{ver_orc_formatado}'.")
        ...
        print(f"[ERRO] actualizar_ids_num_orc_ver_orc_tab_modulo_medidas: Erro inesperado: {e}")
        import traceback
        traceback.print_exc()
    finally:
...
def show_context_menu(ui, pos):
    ...
    action_copy = menu.addAction("Copiar Linha(s) (Não Implementado)") # Manter como placeholder
