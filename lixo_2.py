...
import tabela_def_pecas_items # Importar o módulo necessário para chamar a função de atualização da tab_modulo_medidas

import modulo_dados_definicoes# Importar o módulo para as funções de carregamento (embora sejam chamadas indiretamente)


from dados_items_materiais import configurar_dados_items_orcamento_materiais as configurar_dados_items_orcamento_materiais #Importa a função para configurar os Dados Items (tabela de materiais do item)
from dados_items_ferragens import configurar_dados_items_orcamento_ferragens as configurar_dados_items_orcamento_ferragens #Importa a função para configurar os Dados Items (tabela de ferragens do item)
from dados_items_sistemas_correr import configurar_dados_items_orcamento_sistemas_correr as configurar_dados_items_orcamento_sistemas_correr #Importa a função para configurar os Dados Items (tabela de sistemas_correr do item)
from dados_items_acabamentos import configurar_dados_items_orcamento_acabamentos as configurar_dados_items_orcamento_acabamentos #Importa a função para configurar os Dados Items (tabela de acabamentos do item)
from modulo_dados_definicoes import carregar_dados_modulo_medidas, carregar_dados_def_pecas #Importa a função para carregar os dados da Base Dados SQL do módulo de medidas e da tabela de definições de peças
...
def configurar_orcamento_ui(main_window):
    ...
    ui.lineEdit_margem_lucro.textChanged.connect(lambda: _validate_percentage_input(ui.lineEdit_margem_lucro, 0, 99))
    ui.lineEdit_custos_administrativos.textChanged.connect(lambda: _validate_percentage_input(ui.lineEdit_custos_administrativos, 0, 99))
    ui.lineEdit_ajustes_1.textChanged.connect(lambda: _validate_percentage_input(ui.lineEdit_ajustes_1, 0, 99))
    ui.lineEdit_ajustes_2.textChanged.connect(lambda: _validate_percentage_input(ui.lineEdit_ajustes_2, 0, 99))


    # Botão "Abrir Orçamento"
    ui.pushButton_abrir_orcamento.clicked.connect(lambda: abrir_orcamento((main_window)))

    # Botões de Inserir, Eliminar,  Editar itens, Pesquisar, Configurar Dados Gerais
    ui.pushButton_inserir_linha_orcamento.clicked.connect(lambda: inserir_item_orcamento(ui))
    ui.pushButton_configurar_dados_gerais.clicked.connect(lambda: configurar_dados_gerais(main_window)) # Preenche os campos num_orc e ver_orc nas tabelas dos Dados Gerais
    ui.pushButton_eliminar_linha_orcamento.clicked.connect(lambda: eliminar_item_orcamento(ui))
    ui.pushButton_editar_linha_orcamento_2.clicked.connect(lambda: editar_item_orcamento_groupbox(ui))
    ui.lineEdit_pesquisar_orcamento.textChanged.connect(lambda: pesquisar_itens(ui))
    ...
    ui.pushButton_atualiza_preco_items.clicked.connect(lambda: atualizar_custos_e_precos_itens(ui, force_global_margin_update=False))
    ...
    ui.pushButton_atualiza_preco_final.clicked.connect(lambda: calcular_preco_final_orcamento(ui))
    ...
    ui.tableWidget_artigos.setColumnHidden(COL_ID_ITEM, True)  # Ocultar coluna id_item

    # Permite edição direta na tabela
    ui.tableWidget_artigos.setEditTriggers(
        QAbstractItemView.DoubleClicked | QAbstractItemView.SelectedClicked
    )
    ui.tableWidget_artigos.itemChanged.connect(lambda item: handle_item_editado(ui, item))

    # Ao selecionar uma linha, atualiza os campos do groupBox para edição
    ui.tableWidget_artigos.itemSelectionChanged.connect(lambda: atualizar_campos_groupbox_por_selecao(ui))
...
def _validate_percentage_input(line_edit, min_val, max_val):
    ...
    try:
        ...
        formatted_text = f"{value:.0f}%" # Formatar como inteiro para percentagem
        ...
        QMessageBox.warning(line_edit.window(), "Entrada Inválida",
                            "Por favor, insira um valor numérico válido para a percentagem (ex: 15, 5%).")
...
def mapeia_dados_items_orcamento(main_window):
    ...
    for row in range(rows):
        item_data = []
        item_data.append(_get_cell_text(source, row, COL_ITEM_NUM))      # Item (source col 1) -> dest col 0
        item_data.append(_get_cell_text(source, row, COL_CODIGO))       # Codigo (source col 2) -> dest col 1
        item_data.append(_get_cell_text(source, row, COL_DESCRICAO))    # Descricao (source col 3) -> dest col 2
        item_data.append(_get_cell_text(source, row, COL_ALTURA))       # Altura (source col 4) -> dest col 3
        item_data.append(_get_cell_text(source, row, COL_LARGURA))      # Largura (source col 5) -> dest col 4
        item_data.append(_get_cell_text(source, row, COL_PROFUNDIDADE)) # Profundidade (source col 6) -> dest col 5
        item_data.append(_get_cell_text(source, row, COL_UND))          # Und (source col 7) -> dest col 6
        item_data.append(_get_cell_text(source, row, COL_QT))           # QT (source col 8) -> dest col 7
        item_data.append(_get_cell_text(source, row, COL_PRECO_UNIT))   # Preco_Unit (source col 9) -> dest col 8
        
        # Coluna 9 (no QTreeWidget) é "New Column" - deixada vazia
        item_data.append("") 

        item_data.append(_get_cell_text(source, row, COL_PRECO_TOTAL)) # Preco_Total (source col 10) -> dest col 10
...
def navegar_item_orcamento(main_window, direcao):
    ...
    print(f"  -> IDs obtidos: Item='{novo_id_str}', NumOrc='{novo_num_orc_str}', VerOrc='{novo_ver_orc_str}'")
    print(f"  -> Dados Tree: {[item.text(i) for i in range(tree.columnCount())]}") # Mostra todos os dados da linha da árvore
    ...
    ui.plainTextEdit_descricao_orcamento.setPlainText(item.text(2)) # Descricao
    ...
    tree.scrollToItem(item, QAbstractItemView.PositionAtCenter) # Rola para o item
    ...
    # # Chama as funções de carregamento (usam os lineEdits que acabamos de atualizar)
    # modulo_dados_definicoes.carregar_dados_modulo_medidas(ui)
    # modulo_dados_definicoes.carregar_dados_def_pecas(ui) # Esta chama atualizar_tudo
    #
    # print(f"[DEBUG navegar_item] Chamando actualizar_ids... com: {novo_id_str}, {novo_num_orc_str}, {novo_ver_orc_str}")
    # # CHAMA A FUNÇÃO MODIFICADA, passando os novos valores explicitamente
    # tabela_def_pecas_items.actualizar_ids_num_orc_ver_orc_tab_modulo_medidas(
    #     ui,
    #     novo_id_str,
    #     novo_num_orc_str,
...
def criar_tabela_orcamento_items():
    ...
    try:
        ...
        QMessageBox.critical(None, "Erro BD", f"Falha na tabela 'orcamento_items':\n{err}")
    except Exception as e:
        print(f"Erro inesperado ao criar/verificar 'orcamento_items': {e}")
        QMessageBox.critical(None, "Erro", f"Falha na configuração inicial:\n{e}")
...
def abrir_orcamento(main_window):
    ...
    if row_sel < 0: QMessageBox.warning(None, "Aviso", "Nenhum orçamento selecionado."); return
    ...
    try:
        ...
        QMessageBox.critical(None, "Erro", "Não foi possível obter IDs do orçamento selecionado.")
        return
    ...
    try:
        ...
        with obter_cursor() as cursor:
            ...
            cursor.execute("SELECT nome, morada, email, telefone FROM clientes WHERE id=%s", (id_cli,))
            row_cli = cursor.fetchone()
        # Commit/Rollback/Fecho automático

        # Preenche UI (fora do 'with')
        if row_orc:
            (ano, num, ver, status, dt_str, preco, ref, obra, carac, loc, i1, i2) = row_orc
            ...
            if ver is not None:
                try:
                    ...
                    versao_int = int(str(ver).strip()) # Converter para string antes para lidar com possíveis tipos diferentes
                    versao_formatada = f"{versao_int:02d}"
                except (ValueError, TypeError):
                    # Se não for um número válido, usa a string original se não for vazia, senão "00"
                    versao_formatada = str(ver).strip() if str(ver).strip() else "00"
            # === FIM MODIFICAÇÃO ===
            ...
            try: # Formata data
                ...
                ui.dateEdit_data_orcamento.setDate(QDate(dt.year, dt.month, dt.day))
            except (ValueError, TypeError): ui.dateEdit_data_orcamento.setDate(QDate.currentDate())
            # Preencher outros campos se necessário (status, etc.)
        else: print(f"Aviso: Orçamento ID {id_orc} não encontrado na BD.")
        ...
        else: print(f"Aviso: Cliente ID {id_cli} não encontrado na BD.")
        ...
        # Define próximo item
        prox = obter_proximo_item_para_orcamento(id_orc) # Chama função refatorada
        ...
        QMessageBox.critical(None, "Erro Base de Dados", f"Erro ao carregar dados do orçamento:\n{err}")
    except Exception as e:
        print(f"Erro inesperado ao abrir orçamento: {e}")
        QMessageBox.critical(None, "Erro Inesperado", f"Erro ao abrir orçamento:\n{e}")
...
def carregar_itens_orcamento(ui, id_orcamento: int):
    ...
    try:
        ...
        print(f"Erro MySQL ao carregar itens: {err}"); return # Não preenche a tabela se houver erro
    except Exception as e:
        print(f"Erro inesperado ao carregar itens: {e}"); return

    tbl = ui.tableWidget_artigos
    global _editando_programaticamente
    _editando_programaticamente = True # Evita que itemChanged seja disparado durante o carregamento
    ...
    try:
            ...
            for row_data in registros:
                row_idx = tbl.rowCount(); tbl.insertRow(row_idx)
                for col_ui_idx in range(tbl.columnCount()):
                    ...
                    if col_ui_idx == COL_ID_ITEM: # id_item (coluna 0)
                        item_q.setFlags(item_q.flags() & ~Qt.ItemIsEditable) # Não editável
                    ...
                    if col_ui_idx in [COL_MARGEM_PERC, COL_CUSTOS_ADMIN_PERC, COL_AJUSTES1_PERC, COL_AJUSTES2_PERC]:
                        ...
                        if col_ui_idx == COL_MARGEM_PERC:
                            global_perc_val = converter_texto_para_valor(ui.lineEdit_margem_lucro.text(), "percentual")
                        elif col_ui_idx == COL_CUSTOS_ADMIN_PERC:
                            global_perc_val = converter_texto_para_valor(ui.lineEdit_custos_administrativos.text(), "percentual")
                        elif col_ui_idx == COL_AJUSTES1_PERC:
                            global_perc_val = converter_texto_para_valor(ui.lineEdit_ajustes_1.text(), "percentual")
                        elif col_ui_idx == COL_AJUSTES2_PERC:
                            global_perc_val = converter_texto_para_valor(ui.lineEdit_ajustes_2.text(), "percentual")

                        cell_val = converter_texto_para_valor(texto_item, "percentual")
                        if abs(cell_val - global_perc_val) > 0.001:  # Se os valores diferem
                            item_q.setBackground(COLOR_MANUAL_EDIT) # Cor amarela para edição manual
                        else:
                            item_q.setBackground(tbl.palette().base().color()) # Cor padrão da tabela (normalmente branca)
...
def inserir_item_orcamento(ui):
    ...
    try:
        ...
        QMessageBox.warning(None, "Erro", "Altura, Largura, Profundidade e QT devem ser numéricos.")
        return
    ...
    margem_lucro_perc = converter_texto_para_valor(ui.lineEdit_margem_lucro.text(), "percentual")
    custos_admin_perc = converter_texto_para_valor(ui.lineEdit_custos_administrativos.text(), "percentual")
    ajustes1_perc = converter_texto_para_valor(ui.lineEdit_ajustes_1.text(), "percentual")
    ajustes2_perc = converter_texto_para_valor(ui.lineEdit_ajustes_2.text(), "percentual")
    ...
    try:
        ...
        carregar_itens_orcamento(ui, id_orc) # Recarrega tabela para ver o novo item
        ...
        QTimer.singleShot(10, lambda: atualizar_custos_e_precos_itens(ui, force_global_margin_update=False))
        
        
        ui.lineEdit_codigo_orcamento.clear(); ui.plainTextEdit_descricao_orcamento.clear()
        ui.lineEdit_altura_orcamento.clear(); ui.lineEdit_largura_orcamento.clear()
        ui.lineEdit_profundidade_orcamento.clear()
        ui.lineEdit_und_orcamento.setText("und")
        ui.lineEdit_qt_orcamento.setText("1")
        prox = obter_proximo_item_para_orcamento(id_orc); ui.lineEdit_item_orcamento.setText(str(prox))
        QMessageBox.information(None, "OK", "Item inserido com sucesso.")

    except mysql.connector.Error as err: print(f"Erro MySQL ao inserir item: {err}"); QMessageBox.critical(None, "Erro BD", f"Erro: {err}")
    except Exception as e: print(f"Erro inesperado ao inserir item: {e}"); QMessageBox.critical(None, "Erro", f"Erro: {e}")
...
def eliminar_item_orcamento(ui):
    ...
    """
    tbl = ui.tableWidget_artigos; row_sel = tbl.currentRow()
    if row_sel < 0: QMessageBox.warning(None, "Erro", "Nenhuma linha selecionada."); return
    
    id_item_ui_str = _get_cell_text(tbl, row_sel, COL_ID_ITEM) # id_item na coluna 0 (oculta)
    item_num_str = _get_cell_text(tbl, row_sel, COL_ITEM_NUM) # 'Item' na coluna 1
    id_orc_str = ui.lineEdit_id.text().strip()
    ver_orc_str = ui.lineEdit_versao_orcamento.text().strip() # Versão do orçamento

    if not id_item_ui_str.isdigit() or not id_orc_str.isdigit(): QMessageBox.warning(None, "Erro", "ID item/orçamento inválido."); return
    ...
    print(f"Eliminando item ID: {item_num_str} do orçamento ID: {id_orc_str}, Versão: {ver_orc_str}...")
    try:
        with obter_cursor() as cursor:
            ...
            print(f"  {cursor.rowcount} registos eliminados da dados_def_pecas para o item {item_num_str}.")
            
            # 2. Depois, eliminar o item da `orcamento_items`
            cursor.execute("DELETE FROM orcamento_items WHERE id_item=%s", (id_item_db,))
            rows_affected = cursor.rowcount
        
        if rows_affected > 0:
            carregar_itens_orcamento(ui, id_orc_str) # Recarrega a tabela orcamento_items
            QMessageBox.information(None, "OK", "Item e dados relacionados eliminados com sucesso.")
            limpar_dados_linha_orcamento(ui) # Limpa groupbox e atualiza próximo item
            
            # Recalcula o preço final do orçamento após eliminação (não força a margem global)
            QTimer.singleShot(10, lambda: atualizar_custos_e_precos_itens(ui, force_global_margin_update=False)) 
        else:
            QMessageBox.warning(None, "Aviso", f"Item ID {item_num_str} não encontrado para eliminar.")

    except mysql.connector.Error as err: print(f"Erro MySQL ao eliminar item: {err}"); QMessageBox.critical(None, "Erro BD", f"Erro: {err}")
    except Exception as e: print(f"Erro inesperado ao eliminar item: {e}"); QMessageBox.critical(None, "Erro", f"Erro: {e}")
...
def editar_item_orcamento_groupbox(ui):
    ...
    id_item_str = _get_cell_text(tbl, row_sel, COL_ID_ITEM) # ID da tabela orcamento_items
    ...
    try:
        ...
        with obter_cursor() as cursor:
            ...
            if res:
                ...
                QMessageBox.warning(None, "Aviso", "Item não encontrado na base de dados para edição.")
                return
        ...
        carregar_itens_orcamento(ui, id_orc) # Recarrega tabela para que os dados do groupbox apareçam
        limpar_dados_linha_orcamento(ui) # Limpa groupbox e atualiza próximo item
    ...
    except mysql.connector.Error as err: print(f"Erro MySQL ao editar item: {err}"); QMessageBox.critical(None, "Erro BD", f"Erro: {err}")
    except Exception as e: print(f"Erro inesperado ao editar item: {e}"); QMessageBox.critical(None, "Erro", f"Erro: {e}")
...
def obter_proximo_item_para_orcamento(id_orcamento: int) -> int:
    ...
    try:
        with obter_cursor() as cursor:
            ...
            cursor.execute("SELECT MAX(CAST(item AS UNSIGNED)) FROM orcamento_items WHERE id_orcamento=%s", (id_orcamento,))
            resultado = cursor.fetchone()
            if resultado and resultado[0] is not None:
                max_item = int(resultado[0])
        return max_item + 1
    except mysql.connector.Error as err: print(f"Erro MySQL ao obter max item: {err}"); return 1
    except Exception as e: print(f"Erro ao obter max item: {e}"); return 1
...
def handle_item_editado(ui, item):
    ...
    colunas_db_map = {
        ...
        COL_PRECO_TOTAL: "preco_total", # Recalculado, mas pode ser salvo se editado manualmente 
    ...
    if campo_db in ["altura", "largura", "profundidade", "qt", "preco_unitario", "preco_total","custo_produzido",
        ...
        try:
            valor_novo_para_db = converter_texto_para_valor(valor_novo_str, "moeda")
            ...
            QMessageBox.warning(None, "Erro", f"Valor inválido ({valor_novo_str}) para {campo_db}. Deve ser numérico.")
            _editando_programaticamente = False
            return
    elif campo_db in ["margem_lucro_perc", "custos_admin_perc", "ajustes1_perc", "ajustes2_perc"]:
        try:
            valor_novo_para_db = converter_texto_para_valor(valor_novo_str, "percentual")
            # Validação de percentagem para células da tabela
            if not (0 <= valor_novo_para_db * 100 <= 99): # Comparar o valor em percentagem (0-99)
            ...
            if col == COL_MARGEM_PERC:
                global_perc_value = converter_texto_para_valor(ui.lineEdit_margem_lucro.text(), "percentual")
            elif col == COL_CUSTOS_ADMIN_PERC:
                global_perc_value = converter_texto_para_valor(ui.lineEdit_custos_administrativos.text(), "percentual")
            elif col == COL_AJUSTES1_PERC:
                global_perc_value = converter_texto_para_valor(ui.lineEdit_ajustes_1.text(), "percentual")
            elif col == COL_AJUSTES2_PERC:
                global_perc_value = converter_texto_para_valor(ui.lineEdit_ajustes_2.text(), "percentual")
            ...
            if abs(valor_novo_para_db - global_perc_value) > 0.001: # Comparar floats com tolerância
                ...
                item.setBackground(tbl.palette().base().color()) # Cor padrão da tabela
        except ValueError:
            QMessageBox.warning(None, "Erro", f"Valor inválido ({valor_novo_str}) para {campo_db}. Deve ser numérico em percentagem.")
            _editando_programaticamente = False
            return
    ...
    print(f"Atualizando item ID {id_item}, campo {campo_db} para '{valor_novo_str}'...")
    try:
        ...
        with obter_cursor() as cursor:
            cursor.execute(f"UPDATE orcamento_items SET `{campo_db}`=%s WHERE id_item=%s", (valor_novo_para_db, id_item))
        ...
        QMessageBox.critical(None, "Erro BD", f"Erro ao atualizar item na tabela: {err}\nPor favor, tente novamente.")
    except Exception as e:
        print(f"Erro inesperado ao editar item na tabela: {e}")
        QMessageBox.critical(None, "Erro", f"Erro ao editar item na tabela:\n{e}")
...
def calcular_e_atualizar_linha_artigo(ui, row_idx, force_global_margin_update=False):
    ...
    if not all([id_item_db_val_str, id_item_orcamento_str, num_orc_str, ver_orc_str]):
        print(f"[AVISO] Linha {row_idx+1}: Dados de identificação do item incompletos (DB ID, Item, Num_Orc, Ver_Orc). Saltando cálculo de custos detalhados.")
        ...
        try:
            for col_idx in [COL_CUSTO_PRODUZIDO, COL_CUSTO_ORLAS, COL_CUSTO_MO, COL_CUSTO_MP, COL_CUSTO_ACABAMENTOS,
                ...
                if item_q: item_q.setToolTip("Cálculo impossível: dados incompletos.")
        finally:
            _editando_programaticamente = False
        return
    ...
    try:
        with obter_cursor() as cursor:
            ...
            """
            cursor.execute(query, (id_item_orcamento_str, num_orc_str, ver_orc_str))
            res = cursor.fetchone()
            ...
            if res and res[0] is not None: # Verifica se há resultados válidos e não são None
                ...
                print(f"[INFO] Nenhum dado de peça encontrado na dados_def_pecas para Item: {id_item_orcamento_str}, Orc: {num_orc_str}, Ver: {ver_orc_str}. Custos serão 0.")

    except mysql.connector.Error as err:
        print(f"Erro MySQL ao buscar dados de peças para item {id_item_orcamento_str}: {err}")
        return # Sai se houver erro na BD
    except Exception as e:
        print(f"Erro inesperado ao buscar dados de peças para item {id_item_orcamento_str}: {e}")
    ...
    global_margem_lucro_perc = converter_texto_para_valor(ui.lineEdit_margem_lucro.text(), "percentual")
    global_custos_admin_perc = converter_texto_para_valor(ui.lineEdit_custos_administrativos.text(), "percentual")
    global_ajustes1_perc = converter_texto_para_valor(ui.lineEdit_ajustes_1.text(), "percentual")
    global_ajustes2_perc = converter_texto_para_valor(ui.lineEdit_ajustes_2.text(), "percentual")
    ...
    if force_global_margin_update:
        ...
        try:
            ...
            item_q_margin_perc.setText(formatar_valor_percentual(margem_lucro_perc))
            item_q_margin_perc.setBackground(tbl.palette().base().color()) # Resetar cor
        finally:
            _editando_programaticamente = False
    else:
        margem_lucro_perc_cell = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_MARGEM_PERC), "percentual")
        margem_lucro_perc = margem_lucro_perc_cell if abs(margem_lucro_perc_cell - global_margem_lucro_perc) > 0.001 else global_margem_lucro_perc

    # Para os outros percentuais, a lógica de priorizar o valor da célula (se diferente do global) é mantida
    custos_admin_perc_cell = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_CUSTOS_ADMIN_PERC), "percentual")
    custos_admin_perc = custos_admin_perc_cell if abs(custos_admin_perc_cell - global_custos_admin_perc) > 0.001 else global_custos_admin_perc

    ajustes1_perc_cell = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_AJUSTES1_PERC), "percentual")
    ajustes1_perc = ajustes1_perc_cell if abs(ajustes1_perc_cell - global_ajustes1_perc) > 0.001 else global_ajustes1_perc

    ajustes2_perc_cell = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_AJUSTES2_PERC), "percentual")
    ajustes2_perc = ajustes2_perc_cell if abs(ajustes2_perc_cell - global_ajustes2_perc) > 0.001 else global_ajustes2_perc
    #print(f"[DEBUG CalcLinha {row_idx+1}] Percentagens utilizadas: Margem={margem_lucro_perc*100:.2f}%, Admin={custos_admin_perc*100:.2f}%, Aj1={ajustes1_perc*100:.2f}%, Aj2={ajustes2_perc*100:.2f}%")

    # Obter quantidade do item (essencial para Preco_Unit e Preco_Total)
    qt_item = converter_texto_para_valor(_get_cell_text(tbl, row_idx, COL_QT), "moeda")
    if qt_item <= 0: qt_item = 1.0 # Evitar divisão por zero e garantir um mínimo
    #print(f"[DEBUG CalcLinha {row_idx+1}] Quantidade do Item (QT): {qt_item}")

    # --- 3. Cálculos dos valores em euros ---
    # Custo Produzido = Soma de todos os custos de produção (Orlas + Mão de Obra + Matéria Prima + Acabamentos)
    custo_produzido_calculado = (total_orlas + total_mao_obra + total_materia_prima + total_acabamentos)
    ...
    preco_base_por_unidade = (custo_produzido_calculado + valor_margem + valor_custos_admin + valor_ajustes1 + valor_ajustes2)
    ...
    _editando_programaticamente = True # Ativa a flag para evitar disparos recursivos de itemChanged
    try:
        tbl.blockSignals(True) # Bloqueia sinais da tabela enquanto atualiza programaticamente
        ...
        set_item(tbl, row_idx, COL_CUSTO_PRODUZIDO, formatar_valor_moeda(custo_produzido_calculado))
        if tbl.item(row_idx, COL_CUSTO_PRODUZIDO): tbl.item(row_idx, COL_CUSTO_PRODUZIDO).setToolTip(tooltip_custo_produzido)
        ...
        set_item(tbl, row_idx, COL_CUSTO_ORLAS, formatar_valor_moeda(total_orlas))
        if tbl.item(row_idx, COL_CUSTO_ORLAS): tbl.item(row_idx, COL_CUSTO_ORLAS).setToolTip(tooltip_orlas)

        tooltip_mo = f"Soma dos custos de Mão de Obra e máquinas de todas as peças deste item: {total_mao_obra:.2f}€"
        set_item(tbl, row_idx, COL_CUSTO_MO, formatar_valor_moeda(total_mao_obra))
        if tbl.item(row_idx, COL_CUSTO_MO): tbl.item(row_idx, COL_CUSTO_MO).setToolTip(tooltip_mo)

        tooltip_mp = f"Soma dos custos de Matéria Prima de todas as peças deste item: {total_materia_prima:.2f}€"
        set_item(tbl, row_idx, COL_CUSTO_MP, formatar_valor_moeda(total_materia_prima))
        if tbl.item(row_idx, COL_CUSTO_MP): tbl.item(row_idx, COL_CUSTO_MP).setToolTip(tooltip_mp)

        tooltip_acab = f"Soma dos custos de Acabamentos de todas as peças deste item: {total_acabamentos:.2f}€"
        set_item(tbl, row_idx, COL_CUSTO_ACABAMENTOS, formatar_valor_moeda(total_acabamentos))
        if tbl.item(row_idx, COL_CUSTO_ACABAMENTOS): tbl.item(row_idx, COL_CUSTO_ACABAMENTOS).setToolTip(tooltip_acab)
        ...
        if item_q_margin_perc_current:
            ...
            if not force_global_margin_update: # Se não estamos forçando, a cor é definida pela diferença com o global
                cell_val_current = converter_texto_para_valor(item_q_margin_perc_current.text(), "percentual")
                if abs(cell_val_current - global_margem_lucro_perc) > 0.001: # Compara o valor atual da célula com o global
                    item_q_margin_perc_current.setBackground(COLOR_MANUAL_EDIT) # Cor amarela para edição manual
                else:
                    item_q_margin_perc_current.setBackground(tbl.palette().base().color()) # Cor padrão da tabela (normalmente branca)
        
        tooltip_valor_margem = f"Valor da Margem de Lucro: {custo_produzido_calculado:.2f}€ * ({margem_lucro_perc*100:.2f}%) = {valor_margem:.2f}€"
        set_item(tbl, row_idx, COL_MARGEM_VALOR, formatar_valor_moeda(valor_margem))
        if tbl.item(row_idx, COL_MARGEM_VALOR): tbl.item(row_idx, COL_MARGEM_VALOR).setToolTip(tooltip_valor_margem)

        tooltip_admin_perc = f"Percentagem de Custos Administrativos: {custos_admin_perc*100:.2f}%"
        set_item(tbl, row_idx, COL_CUSTOS_ADMIN_PERC, formatar_valor_percentual(custos_admin_perc))
        if tbl.item(row_idx, COL_CUSTOS_ADMIN_PERC): tbl.item(row_idx, COL_CUSTOS_ADMIN_PERC).setToolTip(tooltip_admin_perc)
        ...
        set_item(tbl, row_idx, COL_CUSTOS_ADMIN_VALOR, formatar_valor_moeda(valor_custos_admin))
        if tbl.item(row_idx, COL_CUSTOS_ADMIN_VALOR): tbl.item(row_idx, COL_CUSTOS_ADMIN_VALOR).setToolTip(tooltip_valor_admin)

        tooltip_ajustes1_perc = f"Percentagem de Ajustes 1: {ajustes1_perc*100:.2f}%"
        set_item(tbl, row_idx, COL_AJUSTES1_PERC, formatar_valor_percentual(ajustes1_perc))
        if tbl.item(row_idx, COL_AJUSTES1_PERC): tbl.item(row_idx, COL_AJUSTES1_PERC).setToolTip(tooltip_ajustes1_perc)

        tooltip_valor_ajustes1 = f"Valor de Ajustes 1: {custo_produzido_calculado:.2f}€ * ({ajustes1_perc*100:.2f}%) = {valor_ajustes1:.2f}€"
        set_item(tbl, row_idx, COL_AJUSTES1_VALOR, formatar_valor_moeda(valor_ajustes1))
        if tbl.item(row_idx, COL_AJUSTES1_VALOR): tbl.item(row_idx, COL_AJUSTES1_VALOR).setToolTip(tooltip_valor_ajustes1)

        tooltip_ajustes2_perc = f"Percentagem de Ajustes 2: {ajustes2_perc*100:.2f}%"
        set_item(tbl, row_idx, COL_AJUSTES2_PERC, formatar_valor_percentual(ajustes2_perc))
        if tbl.item(row_idx, COL_AJUSTES2_PERC): tbl.item(row_idx, COL_AJUSTES2_PERC).setToolTip(tooltip_ajustes2_perc)

        tooltip_valor_ajustes2 = f"Valor de Ajustes 2: {custo_produzido_calculado:.2f}€ * ({ajustes2_perc*100:.2f}%) = {valor_ajustes2:.2f}€"
        set_item(tbl, row_idx, COL_AJUSTES2_VALOR, formatar_valor_moeda(valor_ajustes2))
        if tbl.item(row_idx, COL_AJUSTES2_VALOR): tbl.item(row_idx, COL_AJUSTES2_VALOR).setToolTip(tooltip_valor_ajustes2)
        ...
        set_item(tbl, row_idx, COL_PRECO_UNIT, formatar_valor_moeda(preco_unit_calculado))
        if tbl.item(row_idx, COL_PRECO_UNIT): tbl.item(row_idx, COL_PRECO_UNIT).setToolTip(tooltip_preco_unit)

        tooltip_preco_total = f"Preço Total: Preço Unitário ({preco_unit_calculado:.2f}€) * Quantidade ({qt_item:.0f}) = {preco_total_calculado:.2f}€"
        set_item(tbl, row_idx, COL_PRECO_TOTAL, formatar_valor_moeda(preco_total_calculado))
        if tbl.item(row_idx, COL_PRECO_TOTAL): tbl.item(row_idx, COL_PRECO_TOTAL).setToolTip(tooltip_preco_total)
        ...
        with obter_cursor() as cursor:
            ...
            cursor.execute(update_query, (
                ...
                margem_lucro_perc, valor_margem,# Salva o valor que foi efetivamente usado no cálculo (pode ser o manual ou o global)
                custos_admin_perc, valor_custos_admin, ajustes1_perc,
                valor_ajustes1, ajustes2_perc, valor_ajustes2,
        ...
        print(f"[ERRO] Erro MySQL ao atualizar linha de artigo {row_idx+1}: {err}")
        # Relança o erro para ser capturado no nível superior, se necessário
        raise
    except Exception as e:
        print(f"[ERRO] Erro inesperado ao atualizar linha de artigo {row_idx+1}: {e}")
        import traceback; traceback.print_exc()
...
def atualizar_custos_e_precos_itens(ui, force_global_margin_update=False):
    ...
    for row_idx in range(tbl.rowCount()):
        try:
            ...
            calcular_e_atualizar_linha_artigo(ui, row_idx, force_global_margin_update)
        except Exception as e:
            print(f"[ERRO] Falha ao processar linha {row_idx+1} na atualização de itens: {e}")
            # CORRIGIDO: Usar um QWidget real como parent para QMessageBox
            QMessageBox.critical(ui.tabWidget_orcamento, "Erro de Cálculo", f"Falha ao calcular item {row_idx+1}: {e}")
...
def calcular_preco_final_orcamento(ui):
    ...
    # 2. Ler o objetivo de preço final (ex: "2400,00 €") e converter para float
    objetivo_str = ui.lineEdit_atingir_preco_final.text().strip()
    objetivo = converter_texto_para_valor(objetivo_str, "moeda") if objetivo_str else 0.0

    # 3. Somar preço total atual e custo produzido total (para calcular margem se necessário)
    soma_precos_atuais = 0.0
    soma_custos_produzidos = 0.0
    for row in range(tbl.rowCount()):
        # Cada linha: pegar coluna COL_PRECO_TOTAL e COL_CUSTO_PRODUZIDO
        preco_total_item = converter_texto_para_valor(_get_cell_text(tbl, row, COL_PRECO_TOTAL), "moeda")
        custo_prod_item = converter_texto_para_valor(_get_cell_text(tbl, row, COL_CUSTO_PRODUZIDO), "moeda")
        soma_precos_atuais += preco_total_item
        soma_custos_produzidos += custo_prod_item

    # 4. Se não houver objetivo definido (zero) ou tabela vazia, basta exibir a soma atual
    if objetivo == 0.0 or tbl.rowCount() == 0:
        # Usa um parent válido para a QMessageBox (por exemplo, a janela da tabela)
        parent_window = tbl.window()
        # Atualiza o campo de preço final com a soma atual
        ui.lineEdit_preco_final_orcamento.setText(formatar_valor_moeda(soma_precos_atuais))
        return

    # 5. Caso haja objetivo, precisamos calcular a margem de lucro (%) necessária.

    # 5.1. Ler as demais percentagens definidas pelo usuário
    custos_admin_perc = converter_texto_para_valor(ui.lineEdit_custos_administrativos.text(), "percentual")
    ajustes1_perc     = converter_texto_para_valor(ui.lineEdit_ajustes_1.text(), "percentual")
    ajustes2_perc     = converter_texto_para_valor(ui.lineEdit_ajustes_2.text(), "percentual")
    outros_percentuais = custos_admin_perc + ajustes1_perc + ajustes2_perc

    # 5.2. Verificar divisão por zero: se não houver custo produzido total, não dá para calcular margem
    if soma_custos_produzidos == 0.0:
        parent_window = tbl.window()
        QMessageBox.warning(
            parent_window,
            "Erro de Cálculo",
            "O custo produzido está a zero. Não é possível calcular a margem necessária."
        )
        # Mas como o campo PREÇO_FINAL deve sempre mostrar o que existe, atualizamos ele mesmo assim:
        ui.lineEdit_preco_final_orcamento.setText(formatar_valor_moeda(soma_precos_atuais))
        return

    # 5.3. Fórmula para margem global:
    #    (Objetivo / SomaCustoProduzido) = 1 + soma de todas as percentagens (margem + admin + ajustes)
    #    => margem_necessaria = (Objetivo / SomaCustoProduzido) - 1 - (custos_admin + ajustes1 + ajustes2)
    margem_necessaria = (objetivo / soma_custos_produzidos - 1 - outros_percentuais)

    # 5.4. Limitar margem entre 0 e 99%
    if margem_necessaria < 0.0:
        margem_necessaria = 0.0
    margem_necessaria = min(margem_necessaria, 0.99)

    # 6. Gravar o valor calculado em lineEdit_margem_lucro (ex: "12%")
    ui.lineEdit_margem_lucro.setText(formatar_valor_percentual(margem_necessaria))

    # 7. Agora, “forçamos” todos os itens a usar essa margem global:
    #    Chamamos a função que recalcula cada linha, passando force_global_margin_update=True
    atualizar_custos_e_precos_itens(ui, force_global_margin_update=True)

    # 8. Depois que cada linha foi recalculada (já com preço_total atualizado),
    #    somamos novamente para exibir o valor final alcançado:
    novo_total = 0.0
    for row in range(tbl.rowCount()):
        novo_total += converter_texto_para_valor(_get_cell_text(tbl, row, COL_PRECO_TOTAL), "moeda")
    ui.lineEdit_preco_final_orcamento.setText(formatar_valor_moeda(novo_total))

    # 9. Opcional: se quiser mostrar uma mensagem informando a margem aplicada,
    #    use um QMessageBox com um parent válido (a janela da tabela ou a própria janela principal).
    #    Se não quiser essa pop-up, basta comentar as linhas abaixo.
    #
    # parent_window = tbl.window()
    # QMessageBox.information(
    #     parent_window,
    #     "Margem Ajustada",
    #     f"Margem de lucro ajustada para {margem_necessaria*100:.2f}%\n" +
    #     f"Preço final aproximado: {formatar_valor_moeda(novo_total)}"
    # )
...
def configurar_context_menu_groupbox(ui):
    ...
    ui.groupBox_linhas_orcamento.customContextMenuRequested.connect(lambda pos: exibir_menu_contexto_groupbox(ui, pos))
#################################################################################################################################
